...
(function (htmlForm, $, undefined) {
  var onObsChangedCheck = function () {
    var whenValueThenDisplaySection = $(this).data(
      "whenValueThenDisplaySection",
    );
    ...
    if (whenValueThenDisplaySection) {
      $.each(whenValueThenDisplaySection, function (ifValue, thenSection) {
        if (val == ifValue) {
          ...
          $(thenSection)
            .find(
              'input[type="hidden"], input:text, input:password, input:file, select, textarea',
            )
            .val("");
          $(thenSection)
  ...
  htmlForm.setupWhenThen = function (
    obsId,
    valueToSection,
    valueToThenJs,
    valueToElseJs,
  ...
  htmlForm.setupObsToggleHandlers = function () {
    ...
    $("input[toggleDim]")
      .change(function () {
        ...
        if ($(this).is(":checked")) {
          $("#" + target + " :input, ." + target + " :input").removeAttr(
            "disabled",
          );
          $("#" + target + ", ." + target).animate({ opacity: 1.0 }, 0);
        } else {
          $("#" + target + " :input, ." + target + " :input").attr(
            "disabled",
            true,
          );
